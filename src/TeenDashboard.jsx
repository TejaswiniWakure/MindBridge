...
const QUOTES = [
  { text: "Every day is a fresh start. You've got this. 💙", author: "MindBridge Inspiration" },
  ...
  { text: "Progress, not perfection. Keep going. 🌱", author: "Growth Mindset" },
  { text: "You are stronger than you think. Breathe. Believe.", author: "Internal Resilience" },
];

// Moods list
const MOOD_OPTIONS = [
  { label: 'Happy',   emoji: '😊', color: '#10B981', bg: '#D1FAE5' },
  { label: 'Calm',    emoji: '😌', color: '#3B82F6', bg: '#DBEAFE' },
  { label: 'Tired',   emoji: '😴', color: '#8B5CF6', bg: '#EDE9FE' },
  { label: 'Anxious', emoji: '😰', color: '#F59E0B', bg: '#FEF3C7' },
  { label: 'Sad',     emoji: '😢', color: '#EF4444', bg: '#FEE2E2' },
];

// 8 Guided Resource categories
const GUIDED_RESOURCES = [
  { id: 'meditation', title: 'Meditation Videos', duration: '12 min', icon: '🧘', desc: 'Guided visual meditations for deep focus and calm.' },
  { id: 'exercise', title: 'Breathing Exercises', duration: '8 min', icon: '💨', desc: 'Controlled breathing patterns to reduce stress.' },
  { id: 'calm-audio', title: 'Calm Audios', duration: '15 min', icon: '🎧', desc: 'Soothing ambient sounds and binaural beats.' },
  { id: 'anxiety', title: 'Anxiety Therapy', duration: '10 min', icon: '🌊', desc: 'CBT-based exercises to manage anxious thoughts.' },
  { id: 'sleep', title: 'Sleep Aid Guides', duration: '20 min', icon: '🌙', desc: 'Wind-down routines for deeper, restful sleep.' },
  { id: 'mood-boost', title: 'Mood Booster Beats', duration: '10 min', icon: '✨', desc: 'Upbeat lo-fi tracks to energize your mind.' },
  { id: 'mindfulness', title: 'Mindfulness Walks', duration: '7 min', icon: '🍃', desc: 'Grounding techniques while staying active outdoors.' },
  { id: 'relaxation', title: 'Relaxation Music', duration: '18 min', icon: '🎵', desc: 'Curated playlists for complete mental unwinding.' },
];

const THERAPIST_DIRECTORIES = [
  { id: 1, name: 'Dr. Priya Sharma', specialty: 'Adolescent CBT & Coping', exp: '12 yrs', rating: 4.9, avatar: '👩‍⚕️', cost: '$80/session', availability: 'Mon, Wed 2:00 PM - 6:00 PM' },
  { id: 2, name: 'Dr. Arjun Mehta', specialty: 'Teen Stress & Family Dynamics', exp: '8 yrs', rating: 4.7, avatar: '👨‍⚕️', cost: '$90/session', availability: 'Tue, Thu 10:00 AM - 4:00 PM' },
  { id: 3, name: 'Dr. Sarah Collins', specialty: 'Social Anxiety & Self-Esteem', exp: '15 yrs', rating: 4.8, avatar: '👩‍⚕️', cost: '$110/session', availability: 'Fridays 9:00 AM - 3:00 PM' },
];
...
const NAV = [
  ...
  { id: 'performance', label: 'Daily Performance',  icon: '🏆' },
...
const breathLabel = { idle: 'Ready', inhale: 'Inhale…', 'hold-in': 'Hold…', exhale: 'Exhale…', 'hold-out': 'Hold…' };
...
function getAICompanionReply(userInput) {
  ...
  if (text.includes('hello') || text.includes('hi') || text.includes('hey'))
    return "Hey there! 👋 Nice to meet you. I'm your AI Companion. How's your day going? I'm here to listen or just chat!";
  if (text.includes('anxious') || text.includes('anxiety') || text.includes('nervous') || text.includes('scared'))
    return "I hear you — feeling anxious can feel really heavy. Let's take a slow breath. Inhale for 4 seconds, hold, then exhale. 💙 Remember, you are safe here. What's causing this feeling right now?";
  if (text.includes('sad') || text.includes('depressed') || text.includes('bad') || text.includes('unhappy'))
    return "I'm really sorry to hear you're feeling down. 💛 Your feelings are valid, and it's okay not to be okay. If you want to talk about it, I'm here. Maybe try a simple 5-4-3-2-1 grounding exercise?";
  if (text.includes('stress') || text.includes('exam') || text.includes('school') || text.includes('study'))
    return "School stress is real! It can feel super overwhelming when tasks pile up. Try breaking things down: pick just one small task to focus on first, then take a short break. 🌱";
  if (text.includes('sleep') || text.includes('tired') || text.includes('insomnia'))
    return "Sleep is key for recharging your mind. If your thoughts are racing, try shutting off screens 30 minutes before bed or listening to a calm audio track under our Wellness tab. 🌙";
  if (text.includes('happy') || text.includes('good') || text.includes('great') || text.includes('fun'))
    return "That's awesome! 🎉 I love hearing when things go well. What made today feel good for you? Storing these positive vibes is super helpful for rainy days.";
  if (text.includes('lonely') || text.includes('alone'))
    return "I understand — feeling lonely is tough. But remember, reaching out like this takes courage. You're not alone, I'm right here. 💙 Want to talk about what's been happening?";
  if (text.includes('angry') || text.includes('anger') || text.includes('mad') || text.includes('frustrated'))
    return "Anger is a natural emotion. Try the 5-second pause: breathe in, count to 5, breathe out. It gives your brain a moment to reset. What triggered this feeling? Let's work through it. 🔥→🌊";
  return "Thanks for sharing that with me. I'm always here to listen. Can you tell me more about how you're feeling? The more I understand, the better I can support you. 💙";
}
...
export default function TeenDashboard({ user, onLogout, createdRoles, activeRole, setActiveRole, parentRequestSent, parentForm, setParentRequestSent }) {
  ...
  // ─── PROFILE COMPLETION (persisted via localStorage) ───
  // A profile is only "complete" if it contains a familyCode (new format).
  // Old profiles without familyCode → force re-setup so a code gets assigned.
  ...
  const handleResetData = () => {
    if (window.confirm("⚠️ Are you absolutely sure you want to clear all your self-care records (mood logs, journals, gratitude jar entries, breathing logs, and assessment histories)? This action is permanent and cannot be undone.")) {
      ...
      alert("🧹 All stored self-care records and progress histories have been successfully deleted from your device.");
    }
  };
  ...
  const [chatMessages, setChatMessages] = useState([
    { sender: 'bot', text: "Hey! 👋 I'm your AI Companion. Tell me how you're feeling today, or ask me anything. Let's talk!" }
  ]);
  ...
  const handleMoodSubmit = () => {
    ...
    setMoodNote('');
    setLoggedAlert(`✅ Mood "${entry.mood}" logged successfully!`);
  ...
  const submitAnswer = (score) => {
    ...
    if (qIndex + 1 >= quizGroup.questions.length) {
      ...
      let level = 'Healthy';
      let desc = 'Wonderful news! Your screening indicates healthy, balanced levels. 🌟 We recommend taking a 5-minute Box Breathing break in the Wellness Hub to keep your focus and energy high, or documenting a happy reflection in your Journal!';
      if (percentage > 70) {
        level = 'Elevated Support Needed';
        desc = 'It looks like you are carrying a lot of weight right now. That is completely okay. 🤗 We recommend listening to a Deep Relaxation session in the Wellness Hub, and considering reaching out to one of our friendly counselors in the Therapist portal. You do not have to walk this path alone!';
      } else if (percentage > 35) {
        level = 'Moderate Range';
        desc = 'Your indicators are slightly elevated. 💛 Try performing a Box Breathing session to calm your nervous system, or listening to one of the Guided Calming Audios in the Wellness Hub. Jotting down your thoughts in the Journal can also help relieve stress!';
      }
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
        ...
        <div style={{ background: 'linear-gradient(135deg, #2563EB 0%, #3B82F6 100%)', borderRadius: '20px', padding: '30px', color: '#fff', position: 'relative', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '16px' }}>
          <div>
            ...
            <h1 style={{ margin: '0 0 8px', fontSize: '1.8rem', fontWeight: '800' }}>Hey, {savedProfile.nickname}! 👋</h1>
            <p style={{ margin: '0 0 14px', fontSize: '0.92rem', opacity: 0.95, fontStyle: 'italic' }}>"{quote.text}" — {quote.author}</p>
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              ...
              <span style={{ background: 'rgba(255,255,255,0.15)', border: '1px solid rgba(255,255,255,0.35)', padding: '4px 12px', borderRadius: '50px', fontSize: '0.78rem', fontWeight: '700', letterSpacing: '0.05em', fontFamily: 'monospace, inherit' }}>🔑 {savedProfile.familyCode || 'No Code'}</span>
            </div>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '14px' }}>
          {[
            { label: 'Mood Entries', val: moodLogs.length, icon: '😊', color: '#10B981' },
            { label: 'Journal Entries', val: journals.length, icon: '📖', color: '#3B82F6' },
            { label: 'Assessments', val: assessmentsHistory.length, icon: '📊', color: '#8B5CF6' },
            { label: 'Breathing Cycles', val: breathingCyclesCompleted, icon: '🌬️', color: '#F59E0B' },
            { label: 'Booked Sessions', val: bookedSessions.length, icon: '👩‍⚕️', color: '#EF4444' },
            { label: 'Streak', val: moodLogs.length > 0 ? '1 Day' : '0 Days', icon: '🔥', color: '#F97316' },
          ].map((s, i) => (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '14px' }}>
          {[
            { label: 'Log Your Mood', desc: 'Track how you feel right now', page: 'mood', icon: '😊' },
            { label: 'Talk to AI', desc: 'Chat with your companion', page: 'ai', icon: '🤖' },
            { label: 'Write Journal', desc: 'Express your thoughts privately', page: 'journal', icon: '📖' },
            { label: 'Take Assessment', desc: 'Check your wellness levels', page: 'assessment', icon: '📊' },
          ].map((a, i) => (
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: '20px', minHeight: 'calc(100vh - 160px)' }}>
      <div style={{ ...cardStyle, display: 'flex', flexDirection: 'column', height: '100%' }}>
        <div style={{ borderBottom: `1px solid ${themeColors.border}`, paddingBottom: '12px', marginBottom: '14px' }}>
          <h3 style={{ margin: 0, fontSize: '1rem', fontWeight: '700' }}>🤖 Companion Chat Session</h3>
          <p style={{ margin: 0, fontSize: '0.72rem', color: '#10B981' }}>● Active listener online</p>
        </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        ...
        <div style={{ ...cardStyle, background: '#EFF6FF', border: '1px solid #93C5FD' }}>
          <h4 style={{ margin: '0 0 6px', color: '#1E40AF', fontSize: '0.88rem' }}>🔒 Strict Privacy Guard</h4>
  ...
  const renderJournal = () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      <div style={{ display: 'flex', gap: '8px' }}>
        {['new', 'history'].map(tab => (
          <button key={tab} onClick={() => setJournalTab(tab)} style={{
            ...
            {tab === 'new' ? '✏️ New Journal' : '📚 My Journals'}
          </button>
        ))}
      </div>
      ...
      {journalTab === 'new' ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <input type="text" placeholder="🔍 Search saved entries..." value={journalSearch} onChange={e => setJournalSearch(e.target.value)} style={{ padding: '12px 16px', borderRadius: '10px', border: `1.5px solid ${themeColors.border}`, background: themeColors.card, color: themeColors.text, outline: 'none', fontFamily: 'inherit' }} />
          {journals.length === 0 ? (
            <div style={cardStyle}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            ...
            <h2 style={{ margin: '8px 0 4px', fontSize: '1.45rem', fontWeight: '800' }}>🍯 Gratitude Jar</h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '24px' }}>
          <div style={cardStyle}>
            ...
            <div style={{ marginTop: '20px', borderTop: `1px solid ${themeColors.border}`, paddingTop: '16px' }}>
              <strong style={{ fontSize: '0.78rem', color: themeColors.subText, display: 'block', marginBottom: '8px' }}>💡 Need inspiration? Try these:</strong>
        ...
        <div style={cardStyle}>
          <h3 style={{ margin: '0 0 14px' }}>✨ Revisit Your Memories</h3>
    ...
    if (newFlipped.length === 2) {
      ...
      if (memoryCards[idx1].value === memoryCards[idx2].value) {
        ...
        if (matchedPairs.length + 2 === memoryCards.length) {
          ...
          setTimeout(() => alert("🎉 Excellent! You matched all memory cards! Match count logged."), 300);
        }
      } else {
  ...
  const handleColorAnswer = (userGuess) => {
    if (userGuess === colorQuestion.isMatch) {
      ...
      if (colorScore + 1 >= 5) {
        ...
        alert("🎉 Awesome! You scored 5 correct in color matching!");
        setColorScore(0);
      }
    } else {
      setColorScore(0);
      ...
      return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          <div>
            ...
            <h2 style={{ margin: '8px 0 4px', fontSize: '1.45rem', fontWeight: '800' }}>🎮 Interactive Wellness Games</h2>
          ...
          <div style={cardStyle}>
            <h3 style={{ margin: '0 0 10px' }}>📊 Your Gamified Self-Care Stats</h3>
      ...
      return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', alignItems: 'center' }}>
          <div style={{ alignSelf: 'flex-start' }}>
            <button onClick={() => setActiveGame(null)} style={{ background: 'none', border: 'none', color: '#2563EB', cursor: 'pointer', fontSize: '0.85rem', fontWeight: '600' }}>← Back to Games list</button>
          </div>
          <div style={{ ...cardStyle, width: '100%', maxWidth: '500px', textAlign: 'center' }}>
            <h3>💨 Deep Breathing Animation</h3>
            ...
            <button
              onClick={() => {
                ...
                alert("🎉 Breathing game cycle complete! Daily breathing cycle logged.");
                setActiveGame(null);
              }}
                  ...
                  onClick={() => {
                    ...
                    setBubblesPopped(prev => {
                      ...
                      if (newCount === bubbles.length) {
                        ...
                        setTimeout(() => alert("🎉 Satisfying Pop! All bubbles popped! Pop score logged."), 300);
                      }
                      return newCount;
                    });
                  }}
    ...
    if (activeGame === 'color') {
      return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', alignItems: 'center' }}>
          ...
          <div style={{ ...cardStyle, width: '100%', maxWidth: '450px', textAlign: 'center' }}>
            <h3>🎨 Cognitive Color Matching</h3>
            ...
            <div style={{ display: 'flex', gap: '14px', justifyContent: 'center' }}>
              <button onClick={() => handleColorAnswer(true)} style={{ ...btnPrimary, background: '#10B981', border: 'none', padding: '10px 24px' }}>Yes, Match! ✅</button>
              <button onClick={() => handleColorAnswer(false)} style={{ ...btnPrimary, background: '#EF4444', border: 'none', padding: '10px 24px' }}>No, Different ❌</button>
            </div>
          </div>
        </div>
      );
        ...
        {assessStep === 'select' && (
          <div>
            <div style={{ ...cardStyle, background: 'linear-gradient(135deg, #EFF6FF, #DBEAFE)', marginBottom: '20px' }}>
              <h2 style={{ color: '#1E40AF' }}>📊 Clinical Screening Assessments</h2>
              <p style={{ color: '#3B82F6', fontSize: '0.88rem', margin: '4px 0 0' }}>Select a diagnostic wellness scale below.</p>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '16px' }}>
              {[
                { type: 'phqa', name: '😔 Depression (PHQ-A)', desc: '9 questions to screen adolescent depressive conditions.' },
                { type: 'gad7', name: '😰 Anxiety (GAD-7)', desc: '7 questions assessing generalized anxiety patterns.' },
                { type: 'pss10', name: '⚡ Stress (PSS-10)', desc: '10 questions calculating perceived stress levels.' },
                { type: 'sdq', name: '🧑‍🤝‍🧑 Behavior & Social (SDQ)', desc: '25 items exploring behavioral traits and social relations.' },
                { type: 'who5', name: '💛 Overall Well-being (WHO-5)', desc: '5 questions measuring subjective psychological wellness.' }
              ].map(item => (
            ...
            <div style={{ ...cardStyle, marginTop: '24px' }}>
              <h3>🕐 Completed Screening Logs</h3>
  ...
  const renderWellnessHub = () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '28px' }}>
      ...
      <div style={{ ...cardStyle, textAlign: 'center' }}>
        <h3>🌬️ Interactive Box Breathing</h3>
      ...
      {/* 8 GUIDED RESOURCES */}
      <div style={cardStyle}>
        <h3>🧘 Guided Wellness Resources</h3>
        ...
        {activeMedTrack && (
          <div style={{ marginTop: '16px', padding: '12px', background: '#DBEAFE', borderRadius: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: '0.8rem', color: '#1D4ED8' }}>▶ Playing: <strong>{activeMedTrack.title}</strong> ({activeMedTrack.duration})</span>
  ...
  const renderTherapist = () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      <div style={{ ...cardStyle, background: 'linear-gradient(135deg, #EFF6FF, #DBEAFE)' }}>
        <h2 style={{ color: '#1E40AF', margin: '0 0 6px' }}>👩‍⚕️ Professional Support Directory</h2>
        ...
        {THERAPIST_DIRECTORIES.map(t => (
          <div key={t.id} style={{ ...cardStyle }}>
            ...
            <div style={{ fontSize: '0.78rem', color: themeColors.subText, marginBottom: '14px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
              <span>⭐ {t.rating} · {t.exp} experience</span>
              <span>💰 {t.cost}</span>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ ...cardStyle, maxWidth: '500px', width: '100%' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: `1px solid ${themeColors.border}`, paddingBottom: '10px', marginBottom: '16px' }}>
              ...
              <button onClick={() => setSelectedTherapist(null)} style={{ background: 'none', border: 'none', fontSize: '1.2rem', cursor: 'pointer', color: themeColors.subText }}>×</button>
            </div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div style={cardStyle}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '20px' }}>
          <div style={{ width: '64px', height: '64px', borderRadius: '50%', background: 'linear-gradient(135deg, #EFF6FF, #DBEAFE)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.8rem' }}>🦊</div>
          <div>
      ...
      {/* Privacy Settings */}
      <div style={cardStyle}>
        <h3 style={{ margin: '0 0 14px' }}>🔒 Privacy & Sharing</h3>
      ...
      {/* Appearance */}
      <div style={cardStyle}>
        <h3 style={{ margin: '0 0 14px' }}>🎨 Appearance</h3>
        <div style={{ display: 'flex', gap: '10px' }}>
          <button onClick={() => setTheme('light')} style={{ padding: '10px 20px', borderRadius: '8px', border: `2px solid ${!isDark ? '#2563EB' : themeColors.border}`, background: !isDark ? '#EFF6FF' : themeColors.bg, color: themeColors.text, cursor: 'pointer', fontWeight: '600', fontSize: '0.85rem', fontFamily: 'inherit' }}>☀️ Light Mode</button>
          <button onClick={() => setTheme('dark')} style={{ padding: '10px 20px', borderRadius: '8px', border: `2px solid ${isDark ? '#2563EB' : themeColors.border}`, background: isDark ? '#1E40AF' : themeColors.bg, color: isDark ? '#fff' : themeColors.text, cursor: 'pointer', fontWeight: '600', fontSize: '0.85rem', fontFamily: 'inherit' }}>🌙 Dark Mode</button>
        </div>
      </div>

      {/* Data Management & Resets */}
      <div style={{ ...cardStyle, border: '1px solid #FECACA', background: isDark ? '#451A1A' : '#FEF2F2' }}>
        <h3 style={{ margin: '0 0 8px', color: '#EF4444' }}>🧹 Data Privacy Controls</h3>
        ...
        >
          🗑️ Clear All Stored Self-Care Data
        </button>
      </div>

      {/* Roles & Logout */}
      <div style={cardStyle}>
        <h3 style={{ margin: '0 0 14px' }}>🚪 Account & Roles</h3>
  ...
  const renderPerformance = () => {
    ...
    if (performancePercent === 100) encouragement = "Incredible! You completed all daily wellness activities today! 🏆 Keep this healthy momentum going.";
    
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
        <div style={{ ...cardStyle, background: 'linear-gradient(135deg, #EFF6FF 0%, #DBEAFE 100%)' }}>
          <h2 style={{ color: '#1E40AF', margin: 0 }}>🏆 Overall Performance of the Day</h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px' }}>
          ...
          <div style={cardStyle}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {[
                { label: 'Select Today\'s Mood', done: checkedInToday, icon: '😊' },
                { label: 'Write a Journal Reflection', done: journaledToday, icon: '📖' },
                { label: 'Practice Box Breathing', done: breatheCompleted, icon: '🌬️' },
                { label: 'Take a Wellness Screening', done: assessmentCompleted, icon: '📊' }
              ].map((item, idx) => (
                <div key={idx} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', background: themeColors.bg, borderRadius: '8px', border: `1px solid ${themeColors.border}` }}>
                  ...
                  <span style={{ fontSize: '0.82rem', fontWeight: '700', color: item.done ? '#10B981' : '#94A3B8' }}>{item.done ? '✓ Done' : '○ Todo'}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div style={{ ...cardStyle, background: isDark ? '#1E293B' : '#F8FAFC', border: `1px solid ${themeColors.border}`, textAlign: 'center', padding: '24px 16px' }}>
          <h3 style={{ margin: '0 0 8px' }}>📄 Need to share this with a doctor or therapist?</h3>
  ...
  // ─── PROFILE SCREENS (shown before dashboard) ───
  if (!profileCompleted) {
    // ── STEP 1: Form ──
    if (profileStep === 'form') {
      return (
        <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #EBF2FF 0%, #DBEAFE 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', fontFamily: "'Poppins', 'Inter', sans-serif" }}>
          <div style={{ background: '#fff', borderRadius: '24px', padding: '40px', maxWidth: '480px', width: '100%', boxShadow: '0 20px 60px rgba(37,99,235,0.12)' }}>
            ...
            <div style={{ textAlign: 'center', marginBottom: '28px' }}>
              ...
              <p style={{ margin: 0, fontSize: '0.85rem', color: '#64748B', lineHeight: '1.5' }}>Quick setup — takes under 30 seconds. Youth &amp; Young Adults (Ages 13–30).</p>
            </div>
    ...
    // ── STEP 2: Success — Show Family Code ──
      ...
      return (
        <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #EBF2FF 0%, #DBEAFE 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', fontFamily: "'Poppins', 'Inter', sans-serif" }}>
          <div style={{ background: '#fff', borderRadius: '24px', padding: '40px', maxWidth: '480px', width: '100%', boxShadow: '0 20px 60px rgba(37,99,235,0.12)' }}>
            ...
            {/* Celebration header */}
            <div style={{ textAlign: 'center', marginBottom: '28px' }}>
              <div style={{ fontSize: '3.5rem', marginBottom: '10px' }}>{savedProfile.avatar} 🎉</div>
            ...
            <div style={{ background: '#F0FDF4', border: '1px solid #BBF7D0', borderRadius: '12px', padding: '16px', marginBottom: '24px' }}>
              <h4 style={{ margin: '0 0 10px', fontSize: '0.85rem', color: '#065F46', fontWeight: '700' }}>📲 How to share with your parent:</h4>
            ...
            <div style={{ background: '#FFF7ED', border: '1px solid #FED7AA', borderRadius: '10px', padding: '12px', marginBottom: '22px', fontSize: '0.78rem', color: '#92400E', lineHeight: '1.5' }}>
              🔒 <strong>Privacy Promise:</strong> Parents can only see <em>summary stats</em> — never your journals, AI chats, or personal notes.
            </div>
